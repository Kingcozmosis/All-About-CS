...
CS2_LOGO = "https://cdn.cloudflare.steamstatic.com/steam/apps/730/header.jpg"
...
class CS2UpdatesCog(commands.Cog):
    ...
    # -------------------------
    # GUI Setup Command (Admin only)
    # -------------------------
    @app_commands.command(
        name="cs2updates",
        description="Setup CS2 update posts (Admin only)"
    )
    async def cs2updates(self, interaction: discord.Interaction):
        if not interaction.user.guild_permissions.administrator:
            await interaction.response.send_message(
                "❌ You must be an admin to run this command.", ephemeral=True
            )
            return
    ...
    @app_commands.command(
        ...
        description="Post the latest CS2 update (Admin only)"
    )
    async def cs2updates_test(self, interaction: discord.Interaction):
        ...
        if str(interaction.guild.id) not in config:
            await interaction.followup.send("❌ CS2 updates not enabled.")
            return

        feed = feedparser.parse(RSS_URL)
        if not feed.entries:
            await interaction.followup.send("❌ Could not find a CS2 update.")
            return
    ...
    async def send_update(self, channel, update, guild_config):
        role_ping = f"<@&{guild_config['role']}>" if guild_config.get("role") else ""
        embed = discord.Embed(
            title=update["title"],
            url=update["link"],
